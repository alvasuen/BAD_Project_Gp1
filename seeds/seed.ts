...
export async function seed(knex: Knex): Promise<void> {
    ...
    await knex("artists").insert([
        { artists_id: 1, artist_name: "Green Day", lyrics_id: 1 },
    ])

    await knex("songs").insert([
        { songs_id: 1, songs_name: "xx", artists_id: 1, lyrics_id: 1, bookmark_count: 0 },
    ])
    await knex("users").insert([
        { users_id: 1, email: "123@gmail.com", username: "jimsadsa", password: await hashPassword('123456') },
    ])
